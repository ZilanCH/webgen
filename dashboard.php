...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        body { font-family: 'Inter', system-ui, sans-serif; margin: 0; background: #050815; color: #e2e8f0; }
        header { padding: 24px; background: linear-gradient(135deg, rgba(0,188,212,0.25), rgba(139,92,246,0.25)); border-bottom: 1px solid #1f2937; }
        h1 { margin: 0; }
        ...
        .container { max-width: 1000px; margin: 0 auto; padding: 20px; display: grid; gap: 16px; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); }
...
<header>
    <div class="row" style="justify-content: space-between;">
        <div>
            ...
            <p class="small-note">Logged in as <strong><?= htmlspecialchars($user['username'], ENT_QUOTES, 'UTF-8') ?></strong> · <?= htmlspecialchars($user['role'], ENT_QUOTES, 'UTF-8') ?></p>
        </div>
        <span class="tag">Owned pages: <?= count($ownedPages) ?></span>
    </div>
    <div class="nav">
...
<div class="container">
    <div class="card">
        ...
        <?php if (empty($ownedPages)): ?>
            <p class="small-note">No owned pages yet. Use the generator to publish your first site.</p>
        <?php else: ?>
            <ul>
        ...
        <?php endif; ?>
        <a class="button primary" href="/index.php">🚀 Open Generator</a>
    </div>

    <div class="card">
        <h2>All generated sites</h2>
